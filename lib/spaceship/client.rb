...
module Spaceship
  class Client
    ...
    def request(method, url_or_path = nil, params = nil, headers = {},is_plist = true, &block)
      headers.merge!(csrf_tokens)
      headers.merge!({ 'User-Agent' => USER_AGENT })
      if url_or_path.start_with? "https://developerservices2.apple.com"
        headers.merge!({ 'Host' => "developerservices2.apple.com" })
        headers.merge!({ 'Accept' => "text/x-xml-plist" })
        headers.merge!({ 'Content-Type' => "text/x-xml-plist" })
        headers.merge!({ 'Connection' => "close" })
        headers.merge!({ 'X-Xcode-Version' => "7.2 (7C68)" })
    ...
    def parse_response(response, expected_key = nil)
      ...
      if response.body
        if response.body["responseId"]
          @requestId = response.body["responseId"]
        elsif response.body["responseID"]
          @requestId = response.body["responseID"]
        end 
      end 
      ...
      if type == nil && content.kind_of?(String) 
    ...
    def encode_params(params, headers,is_plist = true)
      if params.kind_of? Hash  
        if is_plist
